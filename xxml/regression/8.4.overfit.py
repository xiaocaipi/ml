...
if __name__ == "__main__":
    ...
    x.shape = -1, 1
    y.shape = -1, 1
#先取 诺干阶  再做线性回归   model2  是ridge  回归
    model_1 = Pipeline([
    ...
    for t in range(2):
        ...
        for i, d in enumerate(d_pool):
            #在这里去设置  几阶的   mdoel 是一个pipeline  给这个pipeline 里面的 poly  就是PolynomialFeatures   给这个函数的参数 就是通过 poly__degree 给  其中poly 是名称  
            #degree 是PolynomialFeatures   函数需要的参数   用__  把poly 和degree 连起来
            ...
            x_hat.shape = -1, 1
            #有一个x  可以进行预测 y  model  是得到的模型 就能对x y 计算 得分
