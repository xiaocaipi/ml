...
if __name__ == "__main__":
    ...
    # 因此，Numpy提供了ndarray(N-dimensional array object) n维的array 对象：存储单一数据类型的多维数组。
    ...
    print b
    # # #
    # # # # 数组大小可以通过其shape属性获得   a  是(6,)  b是(3, 4)
    print a.shape
    print b.shape
    # # #
    # # # 也可以强制修改shape
    ...
    # # #
    # # # 当某个轴为-1时，将根据数组元素的个数自动计算此轴的长度  总数是12  有一个是2 那么-1 占位的就是6
    b.shape = 2, -1
    print b
    print b.shape
    # # #
    ...
    print b.dtype
    # # #
    # # # # 可以通过dtype参数在创建时指定元素类型  np.complex是复数的
    ...
    # # 下面函数创建起始值为10^1，终止值为10^2，有10个数的等比数列  默认base 是10 
    # d = np.logspace(1, 2, 10, endpoint=True)
    # print d
    # # #
    # # # 下面创建起始值为2^0，终止值为2^10(包括)，有10个数的等比数列  这里的base设置为2
...
#     plt.show()
    ...
    y_boost = np.exp(-x)
    # 0 1 损失  小于0就是1  大于0 就是0
    y_01 = x < 0
    #hinge 的线就是 1-x  把小于0的 都变成0 
    y_hinge = 1.0 - x
    y_hinge[y_hinge < 0] = 0 
